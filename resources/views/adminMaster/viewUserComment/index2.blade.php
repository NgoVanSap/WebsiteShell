        ...
        <div class="col-xl-9 col-xxl-9 m-b-30" style=" background: white; padding: 32px; ">
            <div class="media align-items-center">
                <div class="bg-img mr-3">
                    <img src="{{ asset('imgProduct/'.$dataCommentProductFirst->image) }}" class="img-fluid" alt="user">
                </div>
                <div>
                    <h4 class="mb-0">{{  $dataDetailCommentFirst->comment_user_name }}</h4>
                    @php
                        $start = \Carbon\Carbon::parse($dataDetailCommentFirst->created_at);
                        $now = \Carbon\Carbon::now();
                        $days_count = $start->diffInDays($now);
                    @endphp
                    <p>
            ...
            <div style="margin-top: 30px;margin-bottom: 56px;">
                <p class="my-4">{{ $dataDetailCommentFirst->comment_information }}</p>
            </div>

            <div class="profile-btn text-center" style="margin-top: 40px;position: absolute;bottom: 15px;width: 98%;">
                <div>
                    <a href="{{ route('viewUserCommentDetailDelete.admin',['id' => $dataDetailCommentFirst->id,'idUser' => $dataDetailCommentFirst->comment_user_id] ) }}" class="btn btn-outline-danger"><i class="fa fa-trash"  title="" data-original-title="Delete"></i></a>
                </div>
            </div>
        </div>
       </div>
