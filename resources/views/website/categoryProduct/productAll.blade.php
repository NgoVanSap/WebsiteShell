                            ...
                            <div class="section-title clearfix">
                                <ul>
                                    ...
                                    <li class="sort-by floatright">
                                        Showing 1-{{ $dataProduct->count() }} of {{ $dataCountProduct }} Results
                                    </li>
                                </ul>
                            </div>
                        </div>
                                ...
                                <div class="col-xs-12 col-sm-6 col-md-4">
                                    <div class="single-product">
                                        <div class="product-img">
                                            @php
                                                    $start = \Carbon\Carbon::parse($dataProducts->created_at);
                                            ...
                                            @endphp
                                             ...
                                             @endif
                                            @if ($dataProducts->price_sale > 0)
                                            <div class="pro-type sell">
                                                <span>sell</span>
                                            </div>
                                            @endif
                                            <a href="{{ route('product.slug.index',['slug' => $dataProducts->slug_name]) }}"><img src="{{ asset('imgProduct/'. $dataProducts->image) }}" alt="Product Title"></a>
                                            <div class="actions-btn">
                                                <input type="hidden" value="{{ $dataProducts->id }}" name="product_id_cart">
                                                <input type="hidden" value="1" name="quantity">
                                                <input type="hidden" value="{{ $dataProducts->price_sale > 0 ? $dataProducts->price_sale : $dataProducts->price }}" name="cart_price">
                                                <input type="hidden" value="{{ $dataProducts->attribute[0]->id }}" name="cart_id_attribute">
                                                <input type="hidden" value="{{ Auth::guard('customer')->id() }}" name="user_id">
                                                <a class="addCart" data-id="{{ $dataProducts->id }}"><i  class="mdi mdi-cart"></i></a>
                                                <a href="#" data-toggle="modal" data-target="#quick-view_{{ $dataProducts->slug_name }}"><i class="mdi mdi-eye"></i></a>
                                                <a style="cursor: pointer" class="addWishList" data-id="{{ $dataProducts->id }}" data-user-id="{{ Auth::guard('customer')->id() }}"><i class="mdi mdi-heart"></i></a>
                                            </div>
                                        </div>
                                        <div class="product-dsc">
                                            <p><a href="{{ route('product.slug.index',['slug' => $dataProducts->slug_name]) }}">{{ $dataProducts->name }}</a></p>
                                            <div class="ratting">
                                            ...
                                            @if ($dataProducts->price_sale > 0)
                                            <span>${{ $dataProducts->price_sale }}
                                                <del>${{ number_format($dataProducts->price,2) }}</del>
                                            </span>
                                            @else
                                            <span>${{ number_format($dataProducts->price ,2)}}</span>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                                            ...
                                            <div class="col-xs-12 col-md-8">
                                                <div class="list-text">
                                                    ...
                                                    <div class="ratting floatright">
                                                        <p>( {{ $dataProducts->comments->count() }} Bình luận )</p>
                                                    </div>
                                                    <h5>
                                                        @if ($dataProducts->price_sale > 0)
                                                            <del>${{ number_format($dataProducts->price,2) }}</del>
                                                            ${{ number_format($dataProducts->price_sale,2) }}
                                                        @else
                                                            ${{ number_format($dataProducts->price,2) }}
                                                        @endif
                                                    </h5>
                                                    <p>{{ $dataProducts->infomation }}</p>
                                                    <div class="list-btn">
                                                        <input type="hidden" value="{{ $dataProducts->id }}" name="product_id_cart">
                                                        <input type="hidden" value="1" name="quantity">
                                                        <input type="hidden" value="{{ $dataProducts->price_sale > 0 ? $dataProducts->price_sale : $dataProducts->price }}" name="cart_price">
                                                        <input type="hidden" value="{{ $dataProducts->attribute[0]->size }}" name="cart_id_attribute">
                                                        <input type="hidden" value="{{ Auth::guard('customer')->id() }}" name="user_id">
                                                        <a class="addCart" data-id="{{ $dataProducts->id }}"><i  class="mdi mdi-cart"></i></a>
                                                        <a style="cursor: pointer" class="addWishList" data-id="{{ $dataProducts->id }}" data-user-id="{{ Auth::guard('customer')->id() }}">yêu thích</a>
                                                        <a href="#" data-toggle="modal" data-target="#quick-view_{{ $dataProducts->slug_name }}">zoom</a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="pagnation-ul">
                                {{ $dataProduct->links('website.categoryProduct.my-pagination.myPagination') }}
                            </div>
                        </div>
                    </div>
                </div>
                                        ...
                                        <div class="quick-right">
                                            <div class="list-text">
                                                ...
                                                <div class="list-btn">
                                                    ...
                                                    <input type="hidden" value="1" name="quantity">
                                                    <input type="hidden" value="{{ Auth::guard('customer')->id() }}" name="user_id">
                                                    <input type="hidden" value="{{ $productDetailModals->attribute[0]->id }}" name="cart_id_attribute">
