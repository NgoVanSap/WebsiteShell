                                ...
                                <li class="panel">
                                    ...
                                    <div class="paypal-dsc panel-collapse collapse" id="collapse2">
                                        <div class="normal-a">
                                            ...
                                            <input type="hidden" value="{{ $dataDanhMucs->id}}" name="categoryId">
                                            @endforeach
                                        </div>
                                    </div>
                                </li>
                            ...
                            <div class="section-title clearfix">
                                <ul>
                                    ...
                                    <li class="sort-by floatright">
                                        Showing 1-{{ $dataProductCategory->count() }} of {{ $dataProductCategoryCount->count() }} Results
                                    </li>
                                </ul>
                            </div>
                        </div>
                    ...
                    <div class="row">
                        <div class="tab-content grid-content">
                            <div class="tab-pane fade in active text-center" id="grid">
                                @foreach ($dataProductCategory as $dataProductCategorys)
                                <div class="col-xs-12 col-sm-6 col-md-4">
                                    <div class="single-product">
                                        <div class="product-img">
                                            @php
                                                    $start = \Carbon\Carbon::parse($dataProductCategorys->created_at);
                                            ...
                                            @endphp
                                             ...
                                             @endif
                                            @if ($dataProductCategorys->price_sale > 0)
                                            <div class="pro-type sell">
                                                <span>sell</span>
                                            </div>
                                            @endif
                                            <a href="{{ route('product.slug.index',['slug' => $dataProductCategorys->slug_name]) }}"><img src="{{ asset('imgProduct/'. $dataProductCategorys->image) }}" alt="Product Title"></a>
                                            <div class="actions-btn">
                                                ...
                                                <a style="cursor: pointer" class="addWishList" data-id="{{ $dataProductCategorys->id }}" data-user-id="{{ Auth::guard('customer')->id() }}"><i class="mdi mdi-heart"></i></a>
                                            </div>
                                        </div>
                                        <div class="product-dsc">
                                            <p><a href="{{ route('product.slug.index',['slug' => $dataProductCategorys->slug_name]) }}">{{ $dataProductCategorys->name }}</a></p>
                                            <div class="ratting">
                                            ...
                                            @if ($dataProductCategorys->price_sale > 0)
                                            <span>${{ $dataProductCategorys->price_sale }}
                                                <del>${{ number_format($dataProductCategorys->price,2) }}</del>
                                            </span>
                                            @else
                                            <span>${{ number_format($dataProductCategorys->price ,2)}}</span>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="single-list-view">
                                        <div class="row">
                                            ...
                                            <div class="col-xs-12 col-md-8">
                                                <div class="list-text">
                                                    <h3>{{ $dataProductCategorys->name }}</h3>
                                                    <span>Summer men’s fashion</span>
                                                    <div class="ratting floatright">
                                                        <p>( {{ $dataProductCategorys->comments->count() }} Comments  )</p>
                                                    </div>
                                                    <h5>
                                                        @if ($dataProductCategorys->price_sale > 0)
                                                            <del>${{ number_format($dataProductCategorys->price,2) }}</del>
                                                            ${{ number_format($dataProductCategorys->price_sale,2) }}
                                                        @else
                                                            ${{ number_format($dataProductCategorys->price,2) }}
                                                        @endif
                                                    </h5>
                                                    <p>{{ $dataProductCategorys->infomation }}</p>
                                                    <div class="list-btn">
                                                        <input type="hidden" value="{{ $dataProductCategorys->id }}" name="product_id_cart">
                                                        <input type="hidden" value="1" name="quantity">
                                                        <input type="hidden" value="{{ $dataProductCategorys->price_sale > 0 ? $dataProductCategorys->price_sale : $dataProductCategorys->price }}" name="cart_price">
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="pagnation-ul">
                                {{ $dataProductCategory->links('website.categoryProduct.my-pagination.myPagination') }}
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="pages error-page section-padding">
                <div class="container text-center">
                    <div class="error-content">
                        <img src="../assetsWebsite1/img/error.png" alt="">
                        <h4>Category No Products.</h4>
