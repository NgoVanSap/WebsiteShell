                            ...
                            <div class="section-title clearfix">
                                <ul>
                                    ...
                                    <li class="sort-by floatright">
                                        Showing 1-{{ $dataProductSale->count() }} of {{ $dataCountProduct }} Results
                                    </li>
                                </ul>
                            </div>
                        </div>
                                ...
                                <div class="col-xs-12 col-sm-6 col-md-4">
                                    <div class="single-product">
                                        <div class="product-img">
                                            @php
                                                    $start = \Carbon\Carbon::parse($dataProductSales->created_at);
                                            ...
                                            @endif
                                            <a href="{{ route('product.slug.index',['slug' => $dataProductSales->slug_name]) }}"><img src="{{ asset('imgProduct/'. $dataProductSales->image) }}" alt="Product Title"></a>
                                            <div class="actions-btn">
                                                <input type="hidden" value="{{ $dataProductSales->id }}" name="product_id_cart">
                                                <input type="hidden" value="1" name="quantity">
                                                <input type="hidden" value="{{ $dataProductSales->price_sale > 0 ? $dataProductSales->price_sale : $dataProductSales->price }}" name="cart_price">
                                                <input type="hidden" value="{{ $dataProductSales->attribute[0]->size }}" name="cart_id_attribute">
                                                <input type="hidden" value="{{ Auth::guard('customer')->id() }}" name="user_id">
                                                <a class="addCart" data-id="{{ $dataProductSales->id }}"><i  class="mdi mdi-cart"></i></a>
                                                <a href="#" data-toggle="modal" data-target="#quick-view_{{ $dataProductSales->slug_name }}"><i class="mdi mdi-eye"></i></a>
                                                <a style="cursor: pointer" class="addWishList" data-id="{{ $dataProductSales->id }}" data-user-id="{{ Auth::guard('customer')->id() }}"><i class="mdi mdi-heart"></i></a>
                                            </div>
                                        </div>
                                        <div class="product-dsc">
                                            <p><a href="{{ route('product.slug.index',['slug' => $dataProductSales->slug_name]) }}">{{ $dataProductSales->name }}</a></p>
                                            <div class="ratting">
                                            ...
                                            @if ($dataProductSales->price_sale > 0)
                                            <span>${{ $dataProductSales->price_sale }}
                                                <del>${{ number_format($dataProductSales->price,2) }}</del>
                                            </span>
                                            @else
                                            <span>${{ number_format($dataProductSales->price ,2)}}</span>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="single-list-view">
                                        <div class="row">
                                            ...
                                            <div class="col-xs-12 col-md-8">
                                                <div class="list-text">
                                                    <h3>{{ $dataProductSales->name }}</h3>
                                                    <span>Summer men’s fashion</span>
                                                    <div class="ratting floatright">
                                                        <p>( {{ $dataProductSales->comments->count() }} Comments )</p>
                                                    </div>
                                                    <h5>
                                                        @if ($dataProductSales->price_sale > 0)
                                                            <del>${{ number_format($dataProductSales->price,2) }}</del>
                                                            ${{ number_format($dataProductSales->price_sale,2) }}
                                                        @else
                                                            ${{ number_format($dataProductSales->price,2) }}
                                                        @endif
                                                    </h5>
                                                    <p>{{ $dataProductSales->infomation }}</p>
                                                    <div class="list-btn">
                                                        <input type="hidden" value="{{ $dataProductSales->id }}" name="product_id_cart">
                                                        <input type="hidden" value="1" name="quantity">
                                                        <input type="hidden" value="{{ $dataProductSales->price_sale > 0 ? $dataProductSales->price_sale : $dataProductSales->price }}" name="cart_price">
                                                        ...
                                                        <input type="hidden" value="{{ Auth::guard('customer')->id() }}" name="user_id">
                                                        <a class="addCart" data-id="{{ $dataProductSales->id }}"><i  class="mdi mdi-cart"></i></a>
                                                        <a style="cursor: pointer" class="addWishList" data-id="{{ $dataProductSales->id }}" data-user-id="{{ Auth::guard('customer')->id() }}">wishlist</a>
                                                        <a href="#" data-toggle="modal" data-target="#quick-view_{{ $dataProductSales->slug_name }}">zoom</a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="pagnation-ul">
                                {{ $dataProductSale->links('website.categoryProduct.my-pagination.myPagination') }}
                            </div>
                        </div>
                    </div>
                </div>
