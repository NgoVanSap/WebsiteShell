                            ...
                            <div class="section-title clearfix">
                                <ul>
                                    ...
                                    <li class="sort-by floatright">
                                        Showing 1-{{ $dataSearchAscendingProduct->count() }} of {{ $dataCountProduct }} Results
                                    </li>
                                </ul>
                            </div>
                        </div>
                    ...
                    <div class="row">
                        <div class="tab-content grid-content">
                            <div class="tab-pane fade in active text-center" id="grid">
                                @foreach ($dataSearchAscendingProduct as $dataSearchAscendingProducts)
                                <div class="col-xs-12 col-sm-6 col-md-4">
                                    <div class="single-product">
                                        <div class="product-img">
                                                    @php
                                                    $start = \Carbon\Carbon::parse($dataSearchAscendingProducts->created_at);
                                            ...
                                            @endif
                                            @if ($dataSearchAscendingProducts->price_sale > 0)
                                            <div class="pro-type sell">
                                                <span>sell</span>
                                            </div>
                                            @endif
                                            <a href="{{ route('product.slug.index',['slug' => $dataSearchAscendingProducts->slug_name]) }}"><img src="{{ asset('imgProduct/'. $dataSearchAscendingProducts->image) }}" alt="Product Title"></a>
                                            <div class="actions-btn">
                                                <input type="hidden" value="{{ $dataSearchAscendingProducts->id }}" name="product_id_cart">
                                                <input type="hidden" value="1" name="quantity">
                                                <input type="hidden" value="{{ $dataSearchAscendingProducts->price_sale > 0 ? $dataSearchAscendingProducts->price_sale : $dataSearchAscendingProducts->price }}" name="cart_price">
                                                <input type="hidden" value="{{ $dataSearchAscendingProducts->attribute[0]->size }}" name="cart_id_attribute">
                                                <input type="hidden" value="{{ Auth::guard('customer')->id() }}" name="user_id">
                                                <a class="addCart" data-id="{{ $dataSearchAscendingProducts->id }}"><i  class="mdi mdi-cart"></i></a>
                                                <a href="#" data-toggle="modal" data-target="#quick-view_{{ $dataSearchAscendingProducts->slug_name }}"><i class="mdi mdi-eye"></i></a>
                                                <a style="cursor: pointer" class="addWishList" data-id="{{ $dataSearchAscendingProducts->id }}" data-user-id="{{ Auth::guard('customer')->id() }}"><i class="mdi mdi-heart"></i></a>
                                            </div>
                                        </div>
                                        <div class="product-dsc">
                                            <p><a href="{{ route('product.slug.index',['slug' => $dataSearchAscendingProducts->slug_name]) }}">{{ $dataSearchAscendingProducts->name }}</a></p>
                                            <div class="ratting">
                                            ...
                                            @if ($dataSearchAscendingProducts->price_sale > 0)
                                            <span>${{ $dataSearchAscendingProducts->price_sale }}
                                                <del>${{ number_format($dataSearchAscendingProducts->price,2) }}</del>
                                            </span>
                                            @else
                                            <span>${{ number_format($dataSearchAscendingProducts->price ,2)}}</span>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="single-list-view">
                                        <div class="row">
                                            ...
                                            <div class="col-xs-12 col-md-8">
                                                <div class="list-text">
                                                    <h3>{{ $dataSearchAscendingProducts->name }}</h3>
                                                    <span>Summer men’s fashion</span>
                                                    <div class="ratting floatright">
                                                        <p>( {{ $dataSearchAscendingProducts->comments->count() }} Comments )</p>
                                                    </div>
                                                    <h5>
                                                        @if ($dataSearchAscendingProducts->price_sale > 0)
                                                            <del>${{ number_format($dataSearchAscendingProducts->price,2) }}</del>
                                                            ${{ number_format($dataSearchAscendingProducts->price_sale,2) }}
                                                        @else
                                                            ${{ number_format($dataSearchAscendingProducts->price,2) }}
                                                        @endif
                                                    </h5>
                                                    <p>{{ $dataSearchAscendingProducts->infomation }}</p>
                                                    <div class="list-btn">
                                                        <input type="hidden" value="{{ $dataSearchAscendingProducts->id }}" name="product_id_cart">
                                                        <input type="hidden" value="1" name="quantity">
                                                        <input type="hidden" value="{{ $dataSearchAscendingProducts->price_sale > 0 ? $dataSearchAscendingProducts->price_sale : $dataSearchAscendingProducts->price }}" name="cart_price">
                                                        ...
                                                        <input type="hidden" value="{{ Auth::guard('customer')->id() }}" name="user_id">
                                                        <a class="addCart" data-id="{{ $dataSearchAscendingProducts->id }}"><i  class="mdi mdi-cart"></i></a>
                                                        <a style="cursor: pointer" class="addWishList" data-id="{{ $dataSearchAscendingProducts->id }}" data-user-id="{{ Auth::guard('customer')->id() }}">wishlist</a>
                                                        <a href="#" data-toggle="modal" data-target="#quick-view_{{ $dataSearchAscendingProducts->slug_name }}">zoom</a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="pagnation-ul">
                                {{ $dataSearchAscendingProduct->links('website.categoryProduct.my-pagination.myPagination') }}
                            </div>
                        </div>
                    </div>
                </div>
