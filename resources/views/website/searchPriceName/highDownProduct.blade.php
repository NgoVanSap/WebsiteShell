                            ...
                            <div class="section-title clearfix">
                                <ul>
                                    ...
                                    <li class="sort-by floatright">
                                        Showing 1-{{ $dataSearchHighDownProduct->count() }} of {{ $dataCountProduct }} Results
                                    </li>
                                </ul>
                            </div>
                        </div>
                    ...
                    <div class="row">
                        <div class="tab-content grid-content">
                            <div class="tab-pane fade in active text-center" id="grid">
                                @foreach ($dataSearchHighDownProduct as $dataSearchHighDownProducts)
                                <div class="col-xs-12 col-sm-6 col-md-4">
                                    <div class="single-product">
                                        <div class="product-img">
                                                    @php
                                                    $start = \Carbon\Carbon::parse($dataSearchHighDownProducts->created_at);
                                            ...
                                            @endif
                                            @if ($dataSearchHighDownProducts->price_sale > 0)
                                            <div class="pro-type sell">
                                                <span>sell</span>
                                            </div>
                                            @endif
                                            <a href="{{ route('product.slug.index',['slug' => $dataSearchHighDownProducts->slug_name]) }}"><img src="{{ asset('imgProduct/'. $dataSearchHighDownProducts->image) }}" alt="Product Title"></a>
                                            <div class="actions-btn">
                                                <input type="hidden" value="{{ $dataSearchHighDownProducts->id }}" name="product_id_cart">
                                                <input type="hidden" value="1" name="quantity">
                                                <input type="hidden" value="{{ $dataSearchHighDownProducts->price_sale > 0 ? $dataSearchHighDownProducts->price_sale : $dataSearchHighDownProducts->price }}" name="cart_price">
                                                <input type="hidden" value="{{ $dataSearchHighDownProducts->attribute[0]->size }}" name="cart_id_attribute">
                                                <input type="hidden" value="{{ Auth::guard('customer')->id() }}" name="user_id">
                                                <a class="addCart" data-id="{{ $dataSearchHighDownProducts->id }}"><i  class="mdi mdi-cart"></i></a>
                                                <a href="#" data-toggle="modal" data-target="#quick-view_{{ $dataSearchHighDownProducts->slug_name }}"><i class="mdi mdi-eye"></i></a>
                                                <a style="cursor: pointer" class="addWishList" data-id="{{ $dataSearchHighDownProducts->id }}" data-user-id="{{ Auth::guard('customer')->id() }}"><i class="mdi mdi-heart"></i></a>
                                            </div>
                                        </div>
                                        <div class="product-dsc">
                                            <p><a href="{{ route('product.slug.index',['slug' => $dataSearchHighDownProducts->slug_name]) }}">{{ $dataSearchHighDownProducts->name }}</a></p>
                                            <div class="ratting">
                                            ...
                                            @if ($dataSearchHighDownProducts->price_sale > 0)
                                            <span>${{ $dataSearchHighDownProducts->price_sale }}
                                                <del>${{ number_format($dataSearchHighDownProducts->price,2) }}</del>
                                            </span>
                                            @else
                                            <span>${{ number_format($dataSearchHighDownProducts->price ,2)}}</span>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="single-list-view">
                                        <div class="row">
                                            ...
                                            <div class="col-xs-12 col-md-8">
                                                <div class="list-text">
                                                    <h3>{{ $dataSearchHighDownProducts->name }}</h3>
                                                    <span>Summer men’s fashion</span>
                                                    <div class="ratting floatright">
                                                        <p>( {{ $dataSearchHighDownProducts->comments->count() }} Comments )</p>
                                                    </div>
                                                    <h5>
                                                        @if ($dataSearchHighDownProducts->price_sale > 0)
                                                            <del>${{ number_format($dataSearchHighDownProducts->price,2) }}</del>
                                                            ${{ number_format($dataSearchHighDownProducts->price_sale,2) }}
                                                        @else
                                                            ${{ number_format($dataSearchHighDownProducts->price,2) }}
                                                        @endif
                                                    </h5>
                                                    <p>{{ $dataSearchHighDownProducts->infomation }}</p>
                                                    <div class="list-btn">
                                                        <input type="hidden" value="{{ $dataSearchHighDownProducts->id }}" name="product_id_cart">
                                                        <input type="hidden" value="1" name="quantity">
                                                        <input type="hidden" value="{{ $dataSearchHighDownProducts->price_sale > 0 ? $dataSearchHighDownProducts->price_sale : $dataSearchHighDownProducts->price }}" name="cart_price">
                                                        ...
                                                        <input type="hidden" value="{{ Auth::guard('customer')->id() }}" name="user_id">
                                                        <a class="addCart" data-id="{{ $dataSearchHighDownProducts->id }}"><i  class="mdi mdi-cart"></i></a>
                                                        <a style="cursor: pointer" class="addWishList" data-id="{{ $dataSearchHighDownProducts->id }}" data-user-id="{{ Auth::guard('customer')->id() }}">wishlist</a>
                                                        <a href="#" data-toggle="modal" data-target="#quick-view_{{ $dataSearchHighDownProducts->slug_name }}">zoom</a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="pagnation-ul">
                                {{ $dataSearchHighDownProduct->links('website.categoryProduct.my-pagination.myPagination') }}
                            </div>
                        </div>
                    </div>
                </div>
