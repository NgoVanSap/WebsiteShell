                ...
                <div class="right-products">
                            ...
                            <div class="section-title clearfix">
                                <ul>
                                    ...
                                    <li class="sort-by floatright">
                                        Showing 1-{{ $product->count() }} of {{ $product->count() }} Results
                                    </li>
                                </ul>
                            </div>
                        </div>
                    ...
                    <div class="row">
                        <div class="tab-content grid-content">
                            @if (isset($product))
                                @if ($product->isEmpty())
                                <div class="searchEmptyProduct" style="text-align: center; margin-top: 100px">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="200" height="200" fill="currentColor" class="bi bi-exclamation-circle" viewBox="0 0 16 16">
                                        <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z"/>
                                        <path d="M7.002 11a1 1 0 1 1 2 0 1 1 0 0 1-2 0zM7.1 4.995a.905.905 0 1 1 1.8 0l-.35 3.507a.552.552 0 0 1-1.1 0L7.1 4.995z"/>
                                      </svg>
                                      <h4 style="margin-top:12px">Search product not found.</h4>
                                      <a href="{{ route('product.all.index') }}">
                                        <p style="text-transform: none;text-decoration: underline;">Shop now.</p>
                                    </a>
                                </div>
                                    ...
                                    <div class="col-xs-12 col-sm-6 col-md-4">
                                        <div class="single-product">
                                            <div class="product-img">
                                                        @php
                                                        $start = \Carbon\Carbon::parse($products->created_at);
                                                ...
                                                @endif
                                                <a href="{{ route('product.slug.index',['slug' => $products->slug_name]) }}"><img src="{{ asset('imgProduct/'. $products->image) }}" alt="Product Title"></a>
                                                <div class="actions-btn">
                                                    <input type="hidden" value="{{ $products->id }}" name="product_id_cart">
                                                    <input type="hidden" value="1" name="quantity">
                                                    <input type="hidden" value="{{ $products->price_sale > 0 ? $products->price_sale : $products->price }}" name="cart_price">
                                                    <input type="hidden" value="{{ $products->attribute[0]->size }}" name="cart_id_attribute">
                                                    <input type="hidden" value="{{ Auth::guard('customer')->id() }}" name="user_id">
                                                    <a class="addCart" data-id="{{ $products->id }}"><i  class="mdi mdi-cart"></i></a>
                                                    <a href="#" data-toggle="modal" data-target="#quick-view_{{ $products->slug_name }}"><i class="mdi mdi-eye"></i></a>
                                                    <a style="cursor: pointer" class="addWishList" data-id="{{ $products->id }}" data-user-id="{{ Auth::guard('customer')->id() }}"><i class="mdi mdi-heart"></i></a>
                                                </div>
                                            </div>
                                            <div class="product-dsc">
                                                <p><a href="{{ route('product.slug.index',['slug' => $products->slug_name]) }}">{{ $products->name }}</a></p>
                                                <div class="ratting">
                                                ...
                                                <div class="col-xs-12 col-md-8">
                                                    <div class="list-text">
                                                        ...
                                                        <div class="ratting floatright">
                                                            <p>( {{ $products->comments->count() }} Comments )</p>
                                                        </div>
                                                        <h5>
                                                            @if ($products->price_sale > 0)
                                                                <del>${{ number_format($products->price,2) }}</del>
                                                                ${{ number_format($products->price_sale,2) }}
                                                            @else
                                                                ${{ number_format($products->price,2) }}
                                                            @endif
                                                        </h5>
                                                        <p>{{ $products->infomation }}</p>
                                                        <div class="list-btn">
                                                            <input type="hidden" value="{{ $products->id }}" name="product_id_cart">
                                                            <input type="hidden" value="1" name="quantity">
                                                            <input type="hidden" value="{{ $products->price_sale > 0 ? $products->price_sale : $products->price }}" name="cart_price">
                                                            ...
                                                            <input type="hidden" value="{{ Auth::guard('customer')->id() }}" name="user_id">
                                                            <a class="addCart" data-id="{{ $products->id }}"><i  class="mdi mdi-cart"></i></a>
                                                            <a style="cursor: pointer" class="addWishList" data-id="{{ $products->id }}" data-user-id="{{ Auth::guard('customer')->id() }}">wishlist</a>
                                                            <a href="#" data-toggle="modal" data-target="#quick-view_{{ $products->slug_name }}">zoom</a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="pagnation-ul">
                                {{ $product->links('website.categoryProduct.my-pagination.myPagination') }}
                            </div>
                        </div>
                    </div>
                </div>
